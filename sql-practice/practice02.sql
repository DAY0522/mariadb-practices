...
-- 예) 2014년 07월 10일
select date_format(max(hire_date), '%Y년 %m일 %d일') as '신입사원 입사일'
from employees;

select date_format(hire_date, '%Y년 %m일 %d일') as '신입사원 입사일'
from employees
...
select year(current_date)-year(max(birth_date))+1 as '제일 어린 사원',
	   year(current_date)-year(min(birth_date))+1 as '제일 많은 사원'
from employees;